/**
 * Cloudflare Worker for tracking crawler visits - CORS FIXED
 */
...
// CORS headers - CRITICAL FOR GITHUB PAGES
const CORS_HEADERS = {
...
export default {
  async fetch(request, env, ctx) {
    ...
    // Default response
    return new Response('Crawler Tracker Worker - CORS Enabled', {
      headers: {
...
async function getLogs(request, env, url) {
  try {
    ...
    console.log('Found', keys.length, 'log keys'); // Debug
    ...
    console.log('Returning', logs.length, 'logs'); // Debug

    // Sort by timestamp (newest first)
    logs.sort((a, b) => new Date(b.ts).getTime() - new Date(a.ts).getTime());

    // IMPORTANT: Return with CORS headers!
    return new Response(JSON.stringify(logs), {
      status: 200,
      headers: {
        'Content-Type': 'application/json',
        'Cache-Control': 'no-store, no-cache, must-revalidate, max-age=0',
        ...CORS_HEADERS  // THIS IS CRITICAL!
      }
    });
  } catch (err) {
    ...
    return new Response(JSON.stringify({ error: err.message, logs: [] }), {
      ...
      headers: {
        'Content-Type': 'application/json',
        ...CORS_HEADERS  // Even errors need CORS!
      }
    });
  }
}
